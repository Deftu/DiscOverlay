<script lang="ts">
    ...
    let speakingStates: { [id: string]: boolean } = $state({  });
    let voiceStates: { [id: string]: VoiceUserState } = $state({  });
    ...
    onMount(async () => {
        ...
        voiceListenerCallback = await listenForVoiceChannel(async (channel) => {
            if (voiceChannel !== null && voiceChannel.id !== channel.id) {
        ...
        speakStartListenerCallback = await listenForSpeakingStart((userId) => {
            speakingStates[userId] = true;
        });

        speakStopListenerCallback = await listenForSpeakingStop((userId) => {
            speakingStates[userId] = false;
        });
    ...
    function getRenderedUsers() {
        return voiceChannel!!.users
            // Filter out users based on settings
            .filter((user) => {
                if (settings?.visibility?.show_speaking_users_only === true) {
                    return speakingStates[user.id];
                }

                if (settings?.visibility?.show_muted_users === false) {
                    return !voiceStates[user.id]?.server_mute && !voiceStates[user.id]?.self_mute;
                }
            ...
            .sort((a, b) => {
                if (settings?.visibility?.show_own_user_first === true) {
                    if (a.id === ownId) {
                        return -1;
                    }
    ...
    function getAvatarUrl(user: VoiceChannelUser) {
        ...
        // If the user has an avatar, return the URL
        if (avatar !== null) {
            return `https://cdn.discordapp.com/avatars/${user.id}/${avatar}.png`;
        }

        // Otherwise, return the default avatar URL
        const numberId = parseInt(user.id, 10);
        return `https://cdn.discordapp.com/embed/avatars/${numberId % 5}.png`;
    }
</script>
...
{#if voiceChannel !== null}
    <div class="overlay-container">
        {#if settings?.visibility?.show_voice_channel_name ?? true}
            <div class="channel-meta">
                ...
                <h1 class="dfg-header-2">{voiceChannel.channel_name}</h1>
            </div>
        {/if}

        <div class="users">
            {#each getRenderedUsers() as user (user.id)}
                <div class="user" class:speaking={speakingStates[user.id]}>
                    <div class="avatar-container" class:disabled={voiceStates[user.id]?.server_mute || voiceStates[user.id]?.server_deaf || voiceStates[user.id]?.self_mute || voiceStates[user.id]?.self_deaf}>
                        <img
                            class="user-avatar"
                    ...
                    <div class="user-meta">
                        {#if settings?.visibility?.show_usernames ?? true}
                            <h1 class="dfg-title-1">
                                {user.name}
                            </h1>
                        {/if}

                        {#if voiceStates[user.id]?.server_mute || voiceStates[user.id]?.self_mute}
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" class:selfmuted={voiceStates[user.id]?.self_mute} class:servermuted={voiceStates[user.id]?.server_mute}>
                        ...
                        {#if voiceStates[user.id]?.server_deaf || voiceStates[user.id]?.self_deaf}
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" class:selfdeaf={voiceStates[user.id]?.self_deaf} class:serverdeaf={voiceStates[user.id]?.server_deaf}>
